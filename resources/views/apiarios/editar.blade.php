...
<div class="form-wrapper">
    <form method="POST" action="{{ route('apiarios.update', $apiario) }}" class="apiario-form row">
        ...
        <div class="col-md-6 col-12 field-cep">
            ...
            <input type="text" class="cep form-control @error('cep') is-invalid @enderror" id="cep" name="cep" value="{{ old('cep', $endereco->cep ?? '') }}" required maxlength="9" placeholder="00000-000"/>
            @error('cep')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
