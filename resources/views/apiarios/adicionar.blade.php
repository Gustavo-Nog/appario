...
@section('content')
        <div class="form-wrapper">
          <form method="POST" action="{{ route('apiarios.store') }}" class="apiario-form row">
            @csrf
            <h1 class="text-center mb-4">Cadastrar Apiário</h1>

            <div class="col-md-6 col-12 field-nome-apiario">
              <label for="nome">Nome do Apiário<span style="color: red">*</span></label>
              <input type="text" class="form-control @error('nome') is-invalid @enderror" name="nome" id="nome" value="{{ old('nome') }}" required placeholder="Digite um nome"/>
              @error('nome')
            ...
            <div class="col-md-6 col-12 field-bairro">
              ...
              <input type="text" class="form-control @error('bairro') is-invalid @enderror" id="bairro" name="bairro" value="{{ old('bairro') }}" required placeholder="Digite seu bairro"/>
              @error('bairro')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-md-6 col-12 field-numero">
              ...
              <input type="text" class="form-control @error('numero') is-invalid @enderror" id="numero" name="numero" value="{{ old('numero') }}" required placeholder="Digite o número do endereço"/>
              @error('numero')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-md-6 col-12 field-cidade">
              ...
              <input type="text" class="form-control @error('cidade') is-invalid @enderror" id="cidade" name="cidade" value="{{ old('cidade') }}" required placeholder="Digite o nome da cidade"/>
              @error('cidade')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-md-6 col-12 field-logradouro">
              ...
              <input type="text" class="form-control @error('logradouro') is-invalid @enderror" id="logradouro" name="logradouro" value="{{ old('logradouro') }}"  required placeholder="Digite o logradouro"/>
              @error('logradouro')
                <div style="color:red;">{{ $message }}</div>
              @enderror      
            </div>
            ...
            <div class="col-12 mt-3 field-submit">
              <div class="row">
                <div class="col-md-6 col-12">
                  <button type="button" class="button-cancelar" onclick="window.location='{{ route('apiarios.index') }}'">Cancelar</button>
                </div>
                <div class="col-md-6 col-12">
